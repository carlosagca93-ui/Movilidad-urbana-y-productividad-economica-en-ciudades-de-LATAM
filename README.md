...
Carga y exploración: importación de librerías (pandas, numpy, seaborn, matplotlib) y revisión inicial de ambos datasets.
Limpieza y preparación: estandarización de nombres de columnas a snake_case, corrección de tipos de datos (fechas y valores numéricos con separadores de miles, comas decimales y símbolos como %).
Extracción de año y filtrado: derivación del año a partir de las fechas y filtrado de los registros correspondientes a 2024.
Resumen de movilidad: cálculo de promedios anuales de las métricas de tráfico por ciudad, dado que el dataset original tiene múltiples registros por ciudad.
Integración de datasets: unión (inner join) de los datos de movilidad y economía usando city y year como claves, conservando solo las ciudades y años presentes en ambas fuentes.
Visualización: boxplot de la distribución de jams_delay, histograma del PIB per cápita por ciudad, y gráfico de barras comparando ambas variables por ciudad.
Exportación y documentación: guardado del dataset final en CSV y redacción de un resumen ejecutivo con hallazgos y recomendaciones.
...
Asegurate de tener los archivos tomtom_traffic.csv y oecd_city_economy.csv disponibles en la ruta /datasets/ (o ajustá las rutas en el notebook según corresponda).
...
Abrí y ejecutá S5_ladb_mobility_economy_project_student.ipynb en Jupyter Notebook o JupyterLab.
